...
import os.path
import sys
#sys.path.insert(0, os.path.abspath('/home/hy/meeting_transcription/3D-Speaker-main'))
...
from utils.utils.utils import ONNXRuntimeError, OrtInferSession, get_logger, read_yaml
from utils.utils.frontend import WavFrontend, WavFrontendOnline
from utils.utils.e2e_vad import E2EVadModel
#from model import FsmnVADStreaming

logging = get_logger()

#print("vad_bin.py is loading")
...
class Fsmn_vad:
    ...
    def __init__(
        ...
        **kwargs,
        ...
        model_file = os.path.join(model_save_dir, "model.onnx")
        if quantize:
            model_file = os.path.join(model_save_dir, "model_quant.onnx")
        if not os.path.exists(model_file):
            ...
            model_vad, kwargs = AutoModel.build_model(model=model_dir,**kwargs)
            
            kwargs["output_dir"] = model_save_dir
            os.makedirs(model_save_dir, exist_ok=True)
            origin_model_path = kwargs.get("model_path")
            shutil.copy(os.path.join(origin_model_path, "config.yaml"), model_save_dir)
            shutil.copy(os.path.join(origin_model_path, "model.pt"), model_save_dir)
            shutil.copy(os.path.join(origin_model_path, "am.mvn"), model_save_dir)
                ...
                for t_offset in range(0, int(feats_len), min(step, feats_len - t_offset)):
                    ...
                    if is_final:
                        ...
                        feats_package = np.pad(
                            feats_package,
                            ((0, 0), (0, pad_length), (0, 0)),  # 在每个维度上的填充量
                            mode='constant',
                            constant_values=0
                        )                    
                    # 提取对应的波形段
                    ...
                    # 如果是最后一段，对波形进行填充
                    if is_final:
                        # 计算期望的波形长度 (6000帧对应的波形长度)
                        expected_wave_length = 6000 * 160 + 240 # 根据实际情况调整
                        ...
                        if pad_wave_length > 0:
                            waveform_package = np.pad(
                                waveform_package,
                                ((0, 0), (0, pad_wave_length)),  # 在第二个维度上填充
                                mode='constant',
                                constant_values=0
                            )

                    if t_offset==6000 or t_offset==feats_len.max():
                        # 创建存储目录,保存量化数据
                        #save_dir = "datasets"
                        ...
                        # 清理原始npy文件
                        import glob
                        for npy_file in glob.glob(os.path.join(self.datasets_dir, "*.npy")):
                            os.remove(npy_file)
